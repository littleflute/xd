...
//初始化
    ...
    function xdGetJsLrc(src){


	xdDbgMsg(src);	

        var lrcFile = blScript("lrcFile",src);  
	var btnLyr = blBtn(jsonFile,"btnLyr","lyric");
	btnLyr.onclick = function(){
			var     lrcVal,
        			lrcArray = [],
        			lrcTimeArray = [],
       				html = "",
			...
			lrcVal = s.replace(/\[\d\d:\d\d.\d\d]/g,"");
